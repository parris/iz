...
(function () {
     ...
     * @param format {Array, RegExp} (optional) if RegExp then it will be the full format,
     * if an array then then length should be 2. The first value is the string before and the second is the string after
     * @return {Boolean}
     */
    function zuul_money(str, format) {
        ...
        if (typeof str === "string" && (format instanceof RegExp
            ...
            if (format instanceof RegExp) {
                ...
                //Based on CakePHP's money validator: http://api.cakephp.org/view_source/validation#line-509
                //format[0] + 0->inf spaces + valid number + 0->inf spaves + format[1]
                re = (/(format[0])[ ]*(?!0,?\d)(?:\d{1,3}(?:([, .])\d{3})?(?:\1\d{3})*|(?:\d+))((?!\1)[,.]\d{2})?[ ]*(format[1])/);
            }
